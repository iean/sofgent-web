...
export default function ContactFormModern() {
  ...
  if (status === "success") {
    return (
      <div
        ...
        tabIndex={-1}
        ref={(el) => el?.focus()}
        className="rounded-[16px] bg-white p-10 flex flex-col items-center text-center focus:outline-none"
        style={{ border: "1px solid #e6e6e6" }}
      >
        <div className="w-[54px] h-[54px] rounded-[14px] flex items-center justify-center mb-4 text-white" style={{ background: "#326d6d" }}>
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><path d="M20 6 9 17l-5-5" /></svg>
        </div>
  ...
  const labelCls = "block text-[12.5px] font-semibold text-[#1a1a1a] mb-1.5";
  const req = (
      ...
      <form onSubmit={onSubmit} noValidate={false}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="mb-4">
            ...
            <input id="firstName" name="firstName" required aria-required="true" autoComplete="given-name" value={form.firstName} onChange={(e) => set("firstName", e.target.value)} placeholder="Jane" className={fieldBase} style={{ border: "1px solid #e6e6e6" }} />
          </div>
          <div className="mb-4">
            <label htmlFor="lastName" className={labelCls}>Last name</label>
            <input id="lastName" name="lastName" autoComplete="family-name" value={form.lastName} onChange={(e) => set("lastName", e.target.value)} placeholder="Doe" className={fieldBase} style={{ border: "1px solid #e6e6e6" }} />
          </div>
        </div>
        <div className="mb-4">
          <label htmlFor="email" className={labelCls}>Work email{req}</label>
          <input id="email" name="email" required aria-required="true" type="email" autoComplete="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="jane@company.com" className={fieldBase} style={{ border: "1px solid #e6e6e6" }} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div className="mb-4">
            <label htmlFor="phone" className={labelCls}>Phone</label>
            <input id="phone" name="phone" type="tel" autoComplete="tel" value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="+44 …" className={fieldBase} style={{ border: "1px solid #e6e6e6" }} />
          </div>
          <div className="mb-4">
            <label htmlFor="company" className={labelCls}>Company</label>
            <input id="company" name="company" autoComplete="organization" value={form.company} onChange={(e) => set("company", e.target.value)} placeholder="Company name" className={fieldBase} style={{ border: "1px solid #e6e6e6" }} />
          </div>
        </div>
        <div className="mb-4">
          <label htmlFor="need" className={labelCls}>What do you need?</label>
          <select id="need" name="need" value={form.need} onChange={(e) => set("need", e.target.value)} className={fieldBase} style={{ border: "1px solid #e6e6e6" }}>
        ...
        <div className="mb-4">
          ...
          <select id="budget" name="budget" value={form.budget} onChange={(e) => set("budget", e.target.value)} className={fieldBase} style={{ border: "1px solid #e6e6e6" }}>
            <option>Under $10k</option><option>$10k – $25k</option><option>$25k – $50k</option><option>$50k+</option><option>Not sure yet</option>
          </select>
        </div>
        <div className="mb-4">
          <label htmlFor="message" className={labelCls}>Tell us about the project</label>
          <textarea id="message" name="message" value={form.message} onChange={(e) => set("message", e.target.value)} placeholder="What problem are you solving? Any timeline or constraints?" className={`${fieldBase} resize-y`} style={{ border: "1px solid #e6e6e6", minHeight: "110px" }} />
        </div>
        ...
        <p aria-live="polite" role={status === "error" ? "alert" : undefined} className="text-[12.5px] text-[#e11d48] mt-3 text-center" style={{ minHeight: status === "error" ? undefined : 0 }}>
          {status === "error" ? "Something went wrong. Please try again or email contact@sofgent.com." : ""}
        </p>
