              ...
              {timelineData.map((item, index) => (
                <div key={index} className="flex-1 relative z-10">
                  <div className="flex flex-col items-center">
                    {/* Timeline Node */}
                    <div className="w-24 h-24 rounded-full bg-white border-4 border-primary flex items-center justify-center shadow-lg mb-4">
                      <div className="text-2xl">{index + 1}</div>
                    </div>

                    {/* Content Card */}
                    <div className="w-full max-w-[200px] rounded-[20px] border border-[#e7e8e9] bg-white p-4 hover:shadow-lg transition-shadow">
                      <p className="text-xs font-semibold text-primary mb-1">{item.week}</p>
